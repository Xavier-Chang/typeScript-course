...
const mergeObj = merge(
  { name: "Xavier", hobbies: ["piano", "jogging"] },
  { age: 35 }
);
...
function courtAndDescribe<T extends Lengthy>(element: T): [T, string] {
  //the fact is that it has length property, whatever it's string or array. But number is not OK as it has no length
...
class DataStorage<T extends string | number | boolean> {
  //generic type no need to limit the type here
  private data: T[] = [];
...
function createCourseGoal(
  ...
  let courseGoal: Partial<CourseGoal> = {}; //all of the property of CourseGoal is optional
...
const names: Readonly<string[]> = ['Xavier', 'Chang']; //set Readonly, not allow to change the properties of this object
// names.push('Lai')
// names.pop();